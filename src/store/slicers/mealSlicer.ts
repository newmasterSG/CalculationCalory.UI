...
const mealsSlice = createSlice({
  ...
  reducers: {
    setMealItems(
      state,
      action: PayloadAction<{ type: MealType; items: MealItem[] }>
    ) {
    ...
    addMealItem(
      state,
      action: PayloadAction<{ type: MealType; item: MealItem }>
    ) {
    ...
    removeMealItem(
      state,
      action: PayloadAction<{ type: MealType; id: number }>
    ) {
...
export const getProductByMealType = (
  state: RootState,
  type: MealType,
  id: number
) => {
  return state.meals[type].find((item) => item.uniqueId === id);
};
