...
const MealTracker: React.FC<MealTrackerProps> = ({ selectedDate }) => {
  ...
  const { data } = useGetDailyLogByUserQuery(getDailyLogByUserQuery);
  ...
  const initializedMeals = useMemo(
    () => ({
      [MealType.BREAKFAST]: Array.isArray(allMeals[MealType.BREAKFAST])
        ? allMeals[MealType.BREAKFAST]
        : [],
      [MealType.LUNCH]: Array.isArray(allMeals[MealType.LUNCH])
        ? allMeals[MealType.LUNCH]
        : [],
      [MealType.DINNER]: Array.isArray(allMeals[MealType.DINNER])
        ? allMeals[MealType.DINNER]
        : [],
    }),
    [allMeals]
  );
  ...
  const totalCalories = useMemo(
    () =>
      Object.values(allMeals).reduce(
        (total, mealItems) =>
          Array.isArray(mealItems)
            ? total +
              mealItems.reduce(
                (mealTotal, item) =>
                  mealTotal + CalculationHelper.calculateCalories(item),
                0
              )
            : total,
        0
  ...
  const deleteItemFromMeal = useCallback(
    async (meal: MealType, id: number) => {
      try {
        await dispatch(removeMealDailyLogAsync({ type: meal, id }));
        setMeals((prevMeals) => ({
          ...prevMeals,
          [meal]: prevMeals[meal].filter((item) => item.uniqueId !== id),
        }));
      } catch (err) {
  ...
  return (
    <Box p={2}>
      <MealList
        ...
        items={meals[MealType.BREAKFAST]}
        deleteItem={deleteItemFromMeal}
      />
      <MealList
        meal={MealType.LUNCH}
        items={meals[MealType.LUNCH]}
        deleteItem={deleteItemFromMeal}
      />
      <MealList
        meal={MealType.DINNER}
        items={meals[MealType.DINNER]}
        deleteItem={deleteItemFromMeal}
      />
    </Box>
  );
